...
Your basic shell, called `grsh` (short for Golden Ram Shell), is basically an interactive loop: it repeatedly prints a prompt `grsh> ` (note the space after the greater-than sign), parses the input, executes the command specified on that line of input, and waits for the command to finish. This is repeated until the user types exit. The name of your final executable should be grsh.
...
#### Submission
You are to submit a single source code for your shell named **grsh.c**. The compiled binary of your shell program will be tested with the required built in commands (cd, path, exit), several typical Unix commands (ls, cat, pwd ...), and several user executables (selected from the examples in the class). 
